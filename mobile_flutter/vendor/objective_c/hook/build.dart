...
String sdkPath(CodeConfig codeConfig) {
  ...
  try {
    ...
    return firstLineOfStdout('xcodebuild', [
      '-version',
      '-sdk',
      target,
      'Path',
...
String firstLineOfStdout(String cmd, List<String> args) {
  ...
  final initialLines = (result.stdout as String)
      .split('\n')
      .where((line) => line.isNotEmpty);
  if (initialLines.isNotEmpty) {
    return initialLines.first;
  }

  final fallbackExecutable = switch (cmd) {
    'xcrun' => '/usr/bin/xcrun',
    'xcodebuild' => '/usr/bin/xcodebuild',
    _ => null,
  };

  if (fallbackExecutable != null && File(fallbackExecutable).existsSync()) {
    result = Process.runSync(fallbackExecutable, args);
    final fallbackLines = (result.stdout as String)
        .split('\n')
        .where((line) => line.isNotEmpty);
  ...
  throw StateError(
    ...
    '(exit ${result.exitCode}) stderr: ${(result.stderr as String).trim()}',
  );
}
